...
审查期间远端出现 `0334ba0 fix: 修复游戏流程与权限相关缺陷`，与本轮修复出自同一区域、方向有重叠但细节不同。已通过 **rebase + 手动解决冲突** 将两端改动**融合**为最终 `1de4bf8`，完整保留两端的优点：

| 功能 | 来源 | 保留 |
|------|------|------|
| `round_in_progress` 拦截 `/td_go`、`/td_roll` 滥用 | 两端 | ✅ |
| `current_round` 移至结算时递增（`_finish_round`） | 本轮本地 | ✅ |
| `cmd_done` 逐目标玩家确认、全部完成才结算 | 本轮本地 | ✅ |
| 指定目标后未指定玩家仍须 Roll | 本轮本地 | ✅ |
| `_is_admin` 归一化 + 中英文兼容 | 本轮本地 | ✅ |
| `admin_ids` 配置兜底管理员 | 远端 | ✅ |
| `round_timeout` 轮次超时自动跳过（防 AFK） | 远端 | ✅ |
| `_to_at_id` 将 At 的 qq 归一化为 int | 远端 | ✅ |
| 题库多分隔符解析（`re.split`） | 远端 | ✅ |
| 指定目标失效时回退 Roll 点选择 | 远端 | ✅ |

> 说明：`round_timeout` 超时触发时直接 `reset_round()`（不递增轮次），与 `_finish_round` 的"结算递增"方向略有差异，但因都是"重置本轮"，功能可接受，保留远端原实现以降低回归风险。
